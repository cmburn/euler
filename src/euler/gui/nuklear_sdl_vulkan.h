/* This is a patched version of the Nuklear SDL2/Vulkan demo, changed to support
 * SDL3 and be reentrant. Depends on euler's vulkan renderer to find shaders. */
...
NK_API struct nk_sdl *nk_sdl_init(euler_vulkan_renderer *renderer,
    SDL_Window *win, VkDevice logical_device, VkPhysicalDevice physical_device,
...
NK_API struct nk_context *nk_sdl_context(struct nk_sdl *sdl);
NK_API void nk_sdl_shutdown(struct nk_sdl *sdl);
NK_API void nk_sdl_font_stash_begin(struct nk_sdl *sdl,
    struct nk_font_atlas **atlas);
NK_API void nk_sdl_font_stash_end(struct nk_sdl *sdl, VkQueue graphics_queue);
NK_API int nk_sdl_handle_event(struct nk_sdl *sdl, SDL_Event *evt);
NK_API VkSemaphore nk_sdl_render(struct nk_sdl *sdl, VkQueue graphics_queue,
    uint32_t buffer_index, VkSemaphore wait_semaphore,
    enum nk_anti_aliasing AA);
NK_API void nk_sdl_resize(struct nk_sdl *sdl, uint32_t framebuffer_width,
...
NK_API void nk_sdl_handle_grab(struct nk_sdl *sdl);
...
struct nk_sdl_device {
	...
	euler_vulkan_renderer *renderer;
};
...
nk_sdl_create_shader(struct nk_sdl_device *dev, const unsigned char *spv_shader,
...
nk_sdl_create_pipeline(struct nk_sdl_device *dev)
{
	...
	size_t vert_len;
	const uint8_t *vert = euler_vulkan_renderer_shader_data(dev->renderer,
	    "nuklear.vert", &vert_len);
	size_t frag_len;
	const uint8_t *frag = euler_vulkan_renderer_shader_data(dev->renderer,
	    "nuklear.frag", &frag_len);

	shader_stages[0] = nk_sdl_create_shader(dev, vert, vert_len,
	    VK_SHADER_STAGE_VERTEX_BIT);
	shader_stages[1] = nk_sdl_create_shader(dev, frag, frag_len,
...
NK_API void
nk_sdl_device_create(struct nk_sdl_device *dev, VkDevice logical_device,
...
NK_INTERN void
nk_sdl_device_upload_atlas(struct nk_sdl_device *dev, VkQueue graphics_queue,
...
NK_API void
nk_sdl_resize(struct nk_sdl *sdl, uint32_t framebuffer_width,
    uint32_t framebuffer_height)
{
...
NK_API
void
nk_sdl_shutdown(struct nk_sdl *sdl)
{
	nk_font_atlas_clear(&sdl->atlas);
	nk_free(&sdl->ctx);
	nk_sdl_device_destroy(&sdl->vulkan);
	memset(&sdl, 0, sizeof(sdl));
}

NK_API void
nk_sdl_font_stash_begin(struct nk_sdl *sdl, struct nk_font_atlas **atlas)
{
...
NK_API void
nk_sdl_font_stash_end(struct nk_sdl *sdl, VkQueue graphics_queue)
{
	struct nk_sdl_device *dev = &sdl->vulkan;
	...
	nk_sdl_device_upload_atlas(&sdl->vulkan, graphics_queue, image, w, h);
...
NK_API void
nk_sdl_handle_grab(struct nk_sdl *sdl)
{
...
NK_API int
nk_sdl_handle_event(struct nk_sdl *sdl, SDL_Event *evt)
{
	struct nk_context *ctx = &sdl->ctx;
...
NK_API
VkSemaphore
nk_sdl_render(struct nk_sdl *sdl, VkQueue graphics_queue, uint32_t buffer_index,
    VkSemaphore wait_semaphore, enum nk_anti_aliasing AA)
{
	    ...
	    dev->pipeline_layout, 0, 1, &dev->uniform_descriptor_set, 0, NULL);
	{
		 ...
		 */
		{
			...
			nk_convert(&sdl->ctx, &dev->cmds, &vbuf, &ebuf,
			    &config);
		}
...
NK_API struct nk_sdl *
nk_sdl_init(euler_vulkan_renderer *renderer, SDL_Window *win,
    VkDevice logical_device, VkPhysicalDevice physical_device,
    uint32_t graphics_queue_family_index, VkImageView *image_views,
    uint32_t image_views_len, VkFormat color_format,
    enum nk_sdl_init_state init_state, VkDeviceSize max_vertex_buffer,
...
{
	(void)init_state;
	struct nk_sdl *sdl = (struct nk_sdl *)malloc(sizeof(struct nk_sdl));
	...
	SDL_GetWindowSizeInPixels(win, &sdl->display_width,
	    &sdl->display_height);
	sdl->vulkan.renderer = renderer;
	nk_sdl_device_create(&sdl->vulkan, logical_device, physical_device,
...
nk_sdl_context(struct nk_sdl *sdl)
{
	return &sdl->ctx;
}
